...
The best test is just testing the scalar field in both mathematica and Gevolution while the particles' effect via $\Phi$ and $\Phi'$ are turns off in both codes, then we expect to get exactly the same solution! Also it is a good check of solver, if it implemented correctly and works well?! It does not work, so we have a problem in out implementation which should be checked more carefully,
