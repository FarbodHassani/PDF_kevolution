...
//**********************
  \end{lstlisting}
  It is super important to know how update the fields in the first loop to be consistent! \\ of course the last main loop which updates the $\zeta$ for half step in the first loop does not seem to work. Instead we use the following procedure at the first loop. We just assume that $\zeta^{-1/2} \approx \zeta^{(0)}$ which is more or less true if we start from high enough redshift, but we use corrector method to get the correct $\zeta^{(1/2)}$ ,
    \be
  \zeta^{(1/2)} = \zeta^{(-1/2)} - \zeta'^{(0)} d \tau
  \ee
  So before starting the loop over the scalar field we update the $\zeta$ to have the value at step $1/2$ and now we can put them into the loop and get all the values which are synched with particles! This is the same as just updating $\zeta$ and $\pi$ without caring about first loop!! Before we updated $\pi'$ first by half step then we updated $\pi$ in next loops we have updated the $\pi$ so these two fields were not synched with particles! But now instead we just make an assumption which is very well under control by corrector method and instead we get the values of the fields at integer steps synched with particles even at first loop! But it is clear that we might make a mistake at first loop because of the fact that we don't have $\Phi'$...
 \\
 At the end we have the following main part,
 \begin{lstlisting}[language=C++,
  basicstyle=\tiny]
  //**********************
//Kessence - LeapFrog:START
...
double a_kess=a;
	for (i=0;i<sim.nKe_numsteps;i++)
	{
	    ...
	    if(cycle==0)
    {
      for (x.first(); x.test(); x.next()) // Since int he first loop zeta_half is taken zero to not make a lot of mistake we take it equal to the half next step of it i.e zeta^0 but just for the first loop!
      {
        zeta_half(x) =zeta_integer(x);
      }
    }
...
It is possible to get nothing! But it is good to solve in Gevolution and mathematica for some special cases like plane symmetric configuration!
 ...
 \paragraph{The evolution comparisons and tests}
...
%\pi'=\zeta- \pi 
%\ee
   \subsubsection{Lets reconsider the equations again}
 ...
 &{ \zeta'=  3 \mathcal{H}w \zeta  -c_s^2  ({ 3 \mathcal{H}^2 - 3 \mathcal{H}' }) \pi   +c_s^2 \nabla^2 \pi }
  ...
  \pi'=\zeta-\mathcal{H} \pi 
  \ee
\subparagraph{fifth equation:}
Now lets consider more terms in the equation as following. The reason we first consider this equation is that our guess is that the error does not come from $\nabla^2 \pi$ term but the term which has $\mathcal{H}'$!
 \be
\zeta^{(n+1)} = \zeta^{(n)} + d \tau ( 3 \mathcal{H} w \zeta^{(n)}  + c_s^2 \nabla^2 \pi^{(n)})
\ee
\be
\pi^{(n+1)} = \pi^{(n)} + d \tau ( \zeta^{(n)} -\mathcal{H} \pi^{(n)}    )
\ee
...
\paragraph{Leap-frog method: Scalar field only equation}
  \paragraph{Euler method: Full equation}
For the Euler we have the following relations in the loops,
\be
\zeta^{(n+1)} = \zeta^{(n)} + d \tau ( c_s^2 \nabla^2 \pi^{(n)})
\ee
\be
\pi^{(n+1)} = \pi^{(n)} + d \tau ( \zeta^{(n)} -\mathcal{H} \pi^{(n)}    )
\ee
 
So we compare the relative error between mathematica solution and Gevolution for Euler solver, for different precision parameters! To do so, we have made a separate Gevolution file which Euler method is implemented
...
\subsection{Comparing class, mathematica and Gevolution with initial condition from class for mathematica}
  ...
  \subsubsection{Comparing stress tensor and field solution from Gevolution with mathematica and class}
  \paragraph{Comparing the class and Gevolution field and stress tensors at z=0 and z=10}
  -If there is a tension we need to solve in mathematica and comparing with our solution! \\
\paragraph{Checking the limit $c_s^2 \to 0$ that we get kessence density as matter density }
-For the limit $c_s^2 \to 0$ check we get matter power from Gevolution compared with class!
  \paragraph{Comparing with mathematica solution}
  comparing the stress tensor and field and $\zeta$
    \paragraph{Comparing with class }
  comparing the stress tensor and field and $\zeta$
